...
def plot_temp_in2days(begin_index, end_index,df,ax):
    ...
    df['payload_energy'] = df['I_Atlas200DK-A'] / 1000 *12.1+ df['I_Atlas200DK-B'] / 1000 *12.1+ df['I_Pi-A'] / 1000 * 5.1
    df['solar_harvested_energy'] = df['MPPT1_Iout'] / 1000 * df['Total_U'] / 1000 + df['MPPT2_Iout'] / 1000 * df['Total_U'] / 1000
    df['comm_energy']=(df['POBC_I_5V']*5+df['XMIT_A_12V']*12 +df['XMIT_B_12V']*12)/1000
    df['total_energy'] = df['Total_I'] / 1000 * df['Total_U'] / 1000 
    ...
    df['solar_harvested_energy'] = df['solar_harvested_energy'].rolling(window=window_size).mean()
    df['total_energy'] = df['total_energy'].rolling(window=window_size).mean()
    df['payload_energy'] = df['payload_energy'].rolling(window=window_size).mean()
    df['comm_energy'] = df['comm_energy'].rolling(window=window_size).mean()

    for area in shadow_areas:
        ax.axvspan(area[0], area[1], alpha=0.3, color='gray')

    ax.plot(df['TIME'], df['solar_harvested_energy'], label='Solar Power')
    ax.plot(df['TIME'], df['total_energy'], label='Total Power')
    ax.plot(df['TIME'], df['payload_energy'], label='Computing Power')
    ax.plot(df['TIME'], df['comm_energy'], label='Communication Power')
    ...
    ax.set_title('(a) Power Varations in 2 Days with Computing Tasks',y=1.05)
...
def plot_temp_in10hs(begin_index, offset,df,ax):
    ...
    for area in shadow_areas:
        ax.axvspan(area[0], area[1], alpha=0.3, color='gray')
    ax.plot(df['TIME'], df['solar_harvested_energy'])
    ax.plot(df['TIME'], df['total_energy'])
    ...
    ax.set_xlabel("Orbital Period Number", fontsize=20)
    ax.set_ylabel("Power(w)", fontsize=20)
    ax.set_title('(b) Power Varations in 9 Hours without Computing Tasks',y=1.05)
...
axs[1].legend(handles=legend_elements,frameon=False,loc='upper right',fontsize=19)
plt.subplots_adjust(hspace=0.5)

# Output Data
outputdir='.'
plt.savefig(outputdir+"/figure10" + '.pdf', dpi=1000)
# plt.show()
